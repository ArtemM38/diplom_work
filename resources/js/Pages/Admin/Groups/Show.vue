...
const groupAthletes = computed(() => props.group?.athletes ?? []);
const allAthletesList = computed(() => props.allAthletes?.data ?? []);
...
const filteredMembers = computed(() => {
    ...
    if (!q) return groupAthletes.value;
    return groupAthletes.value.filter((athlete) =>
                    ...
                    <form @submit.prevent="addAthlete" class="space-y-4">
                        ...
                        <div>
                            ...
                            <select v-model="form.athlete_id"
                                ...
                                <option v-for="ath in allAthletesList" :key="ath.id" :value="ath.id">
                                    {{ ath.last_name_nom }} {{ ath.first_name_nom }}
                                </option>
                            </select>
                            <Pagination class="mt-3" :links="allAthletes.links" :meta="allAthletes" />
                            <p v-if="athleteSearch && allAthletesList.length === 0" class="text-xs text-gray-400 mt-1">
